...
namespace NewsAggregator.WebUI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> ChangeName(
            [FromBody] UserChangeNameRequest userChangeNameRequest
        )
        {
            ...
            var result = await _userService.ChangeNameAsync(
                userChangeNameRequest.UserName,
                User.GetUserId()
            );
            if (!result.Success)
            {
            ...
            return Ok(new UserChangeNameRequest { UserName = userChangeNameRequest.UserName });
        }
